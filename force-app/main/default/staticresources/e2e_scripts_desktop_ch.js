...
var regs = {
    phone: /^([0-9+]{3}[0]{0,1}[0-9]{9})+$/,
    numbers: /^[0-9]+$/,
    taxId: /^([Cc]{1}[Hh]{1}[Ee]{1}[0-9]{9})+$/,
    letters: /^[a-zA-ZÀ-ž\x7f-\xff\s'-]+$/,
    city: /^[a-zA-ZÀ-ž\x7f-\xff\s'.-]+$/,
    ...
    email: /^\w+([.-]\w+)*@\w+([.-]\w+)*(\.\w{2,4})+$/,
    carNumberFormat_DE: /^[A-Za-z]{1,3}\-?\s?[A-Za-z]{0,2}[0-9]{1,4}[H]{0,1}/,
    swiftFormat: /^([A-Za-z]{6}[A-Za-z0-9]{2,5})+$/,
    ibanFormat: /^[Cc]{1}[Hh]{1}[0-9]{2}[ ]{0,1}[0-9]{4}[ ]{0,1}[0-9]{4}[ ]{0,1}[0-9]{4}[ ]{0,1}[0-9]{4}[ ]{0,1}[0-9]{1}?$/,
    vatIdFormat: /^([Cc]{1}[Hh]{1}[Ee]{1}[0-9]{9})+$/
};
...
var dataSource = 'eurolocator.fleetcor.ch';
var favStationInfo = '';
var errors = {
    0 :'ZERO_SEARCH_RESULTS'
}
var allowBackButtonClick = true;
...
$j(document).ready(function () {
    ...
    phoneInput.inputmask({mask:"+4\\19999999999", "placeholder": "" }); //using jqueryInputMask.resource
    ...
    if(taxIdInput.val() != '' && taxIdInput.val() != undefined){
        validateTaxId(taxIdInput, new RegExp(regs.taxId));
    }

    var clientCountryInput = creditScoringStepBlock.find('.city-id').find('input');
    if (clientCountryInput.val() != '' && clientCountryInput.val() != undefined) {
        isInputValueValid(clientCountryInput, new RegExp(regs.city));
    }
        ...
        if(cardOwnerInput.val() != '' && cardOwnerInput.val() != undefined){
            if(!(selectedCardOwnerType == undefined)){
                if(selectedCardOwnerType.toLowerCase() === 'car-number'){
                    validateCarNumber(cardOwnerInput, new RegExp(regs.carNumberFormat_DE));
                }
                if(selectedCardOwnerType.toLowerCase() === 'driver-name'){
    ...
    /* STEP CARD CONFIGURATION. INPUT VAT ID */
    var inputVatIdBlock = content.find('.enter-vat-id');
    inputVatIdBlock.find('.btn-red').attr('disabled', true);

    var vatIdInput = inputVatIdBlock.find('.vat-number').find('input');
    if (vatIdInput.val() != '' && vatIdInput.val() != undefined) {
        isInputValueValid(vatIdInput, new RegExp(regs.vatIdFormat));
    }

    setEvents_vatNumber();


    /* STEP CREDIT SCORE. ENTER TAX ID */
    var inputTaxIdBlock = content.find('.enter-tax-id');
    inputTaxIdBlock.find('.btn-red').attr('disabled', true);

    var taxIdInput = inputTaxIdBlock.find('.tax-number').find('input');
    if (taxIdInput.val() != '' && taxIdInput.val() != undefined) {
        isInputValueValid(taxIdInput, new RegExp(regs.taxId));
    }
    ...
    setEventsMustRemoveBlock(); // it will be removed in future
...
function validatePumpInputValue(inputElement, regex) {
    if (inputElement.val() != undefined) {
        // Numbers only
        inputElement.val(inputElement.val().replace(/[^\d]/,''));
        $j('.totalPumpField').val(inputElement.val());
    }
    ...
    if((elementValue != undefined) && (elementValue.trim() != '') && regex.test(elementValue)){
        if(pumpValue < minPumpValue) {
            ...
            inputElement.closest('.wrapper').find('.consumption-warning1').show();
            inputElement.closest('.wrapper').find('.consumption-warning2').hide();
        ...
        if(pumpValue > maxPumpValue) {
            inputElement.closest('.form-group input').addClass('error');
            inputElement.closest('.wrapper').find('.consumption-warning2').show();
            inputElement.closest('.wrapper').find('.consumption-warning1').hide();
            inputElement.closest('.wrapper').find('.limit-info').css("color", '#EE3A43');
            inputElement.closest('.wrapper').find('.btn-red').attr('disabled', true);
        } else {
...
function validateCardsTechParams(index) {
    ...
    if(cardOwnerInput.val() != undefined){
        if(selectedCardOwnerType != undefined){
            if(selectedCardOwnerType.toLowerCase() === 'car-number'){
                validatedFields.push(validateCarNumber(cardOwnerInput, new RegExp(regs.carNumberFormat_DE)));
            }
            if(selectedCardOwnerType.toLowerCase() === 'driver-name'){
                validatedFields.push(validateDriverName(cardOwnerInput, new RegExp(regs.embossingLetters)));
            }
        }
    }
...
function validateCarNumber(element, regex) {
    ...
    if((elementValue.trim() != '') && regex.test(elementValue)){
        if(numLength >= 8 && numLength <= 11){
            element.closest('.form-group input').removeClass('error');
            element.closest('.form-group input').addClass('success');
            element.closest('.form-group').find('.invalid-feedback-car-number').hide();
            element.closest('.form-group').find('.glyphicon').show();
            isValid = true;
        } else {
            element.closest('.form-group input').removeClass('success');
            element.closest('.form-group input').addClass('error');
            element.closest('.form-group').find('.invalid-feedback-car-number').show();
            element.closest('.form-group').find('.glyphicon').hide();
            isValid = false;
        }
    } else {
...
function validateDriverName(element, regex) {
    ...
    if((elementValue.trim() != '') && regex.test(elementValue)){
        if(numLength <= 16){
            element.closest('.form-group input').removeClass('error');
            element.closest('.form-group input').addClass('success');
            element.closest('.form-group').find('.invalid-feedback-driver').hide();
            element.closest('.form-group').find('.glyphicon').show();
            isValid = true;
        } else {
            element.closest('.form-group input').removeClass('success');
            element.closest('.form-group input').addClass('error');
            element.closest('.form-group').find('.invalid-feedback-driver').show();
            element.closest('.form-group').find('.glyphicon').hide();
            isValid = false;
        }
    } else {
...
function createSlider(destination, dispOutput, idInputHidden, startVal, minVal, maxVal){
    $j('#' + destination).slider({
        ...
        stop: function(event, ui) {
            var creditLimitOutput = $j('.creditLimitOutput').text();
            analytics_onClickSlideSendUI('kreditlimit_' + creditLimitOutput, 'UI_04_Tankkarte_Step_4_Form_PaymentMethod');
            recalculateDeposit();
            colorSliderActiveLine();
        }
    });
...
function printDocument(frameId, docId, frameWrap) {
    var contentFullSize = $j('#' + frameId).contents().find('#' + docId).height();
    var originalContentSize = $j('#' + frameId).contents().height();
    // set full iframe size
    $j('#' + frameId).contents().find('.mainform').attr('style','height: ' + contentFullSize);
    $j('.' + frameWrap).attr('style','height: ' + contentFullSize);
    // set scroll top
    $j('#' + frameId).contents().find('.mCSB_container').attr('style', 'position: relative; top: 0px; left: 0px;');
    // printing
    document.getElementById(frameId).contentWindow.print();
    // back to original iframe size
    $j('#' + frameId).contents().find('.mainform').attr('style','height: ' + originalContentSize + 'px');
    $j('.' + frameWrap).attr('style','height: ' + originalContentSize + 'px');
}
...
function setEvents_contactDetails() {
    ...
    var numberOfCardsInput = contactDetailsStepBlock.find('.cardsCounterContactDetails');
    numberOfCardsInput.on('change', function (e){
        if (numberOfCardsInput.val() == undefined || numberOfCardsInput.val() == 0 || numberOfCardsInput.val() == null) {
            numberOfCardsInput.val('1');
        }
    });
...
function setEvents_companyDetails() {
    ...
    clientCountryInput.on('input', function(e){
        isInputValueValid(clientCountryInput, new RegExp(regs.city));
    });

    // Commented 09/07 by Eugene / Reason: Block was moved to additional step
    // var taxIdInput = creditScoringStepBlock.find('.tax-id').find('input');
    // if(taxIdInput.val() != '' && taxIdInput.val() != undefined){
    //     validateTaxId(taxIdInput, new RegExp(regs.taxId));
    // }
...
function setEvents_fuelConsumption() {
    ...
    validatePumpInputValue(pumpInput, new RegExp(regs.numbers)); // use for set default attributes
...
function checkCompanyName() {
    ...
    if (cardTechParamsItems.find('.company-name').find('input').val() != '' &&
        ...
        var elementValue = cardTechParamsItems.find('.company-name').find('input').val();
        ...
        if((elementValue.trim() != '') && regex.test(elementValue) && cardTechParamsItems.find('.company-name').find('input').val().length <= 25) {
            isValid = true;
...
function setEvents_vatNumber() {
    ...
    var vatNumberInput = inputVatIdBlock.find('.vat-number').find('input');
    vatNumberInput.on('input', function(e) {
        isInputValueValid(vatNumberInput, new RegExp(regs.vatIdFormat));
        isVatValid = isInputValueValid(vatNumberInput, new RegExp(regs.vatIdFormat));
        if (isVatValid) {
            inputVatIdBlock.find('.btn-red').attr('disabled', false);
        } else {
            inputVatIdBlock.find('.btn-red').attr('disabled', true);
        }
    });
...
function setEvents_taxNumber() {
    var content = $j('.step-content');
    var inputTaxIdBlock = content.find('.enter-tax-id');
    inputTaxIdBlock.find('.btn-red').attr('disabled', true);
    var isTaxValid = false;

    var taxNumberInput = inputTaxIdBlock.find('.tax-number').find('input');
    taxNumberInput.on('input', function(e) {
        validateTaxId(taxNumberInput, new RegExp(regs.numbers));
        isTaxValid = validateTaxId(taxNumberInput, new RegExp(regs.taxId));
        if (isTaxValid) {
            inputTaxIdBlock.find('.btn-red').attr('disabled', false);
        } else {
            inputTaxIdBlock.find('.btn-red').attr('disabled', true);
        }
    });
...
function setEvents_additional() {
    let content = $j('.step-content');
    let additionalStepBlock = content.find('.additional-page');
    let additionalTC = additionalStepBlock.find('.form-check-input.additionalTC');
    let additionalBehalfOfCompany = additionalStepBlock.find('.form-check-input.additionalBehalfOfCompany');
    // check if the elements exist on visualforce (rendered)
    if (additionalTC.length == 1 && additionalBehalfOfCompany.length == 1) {
        // onload
        if (! additionalTC.is(':checked') || ! additionalBehalfOfCompany.is(':checked')) {
            additionalStepBlock.find('.btn-red').attr('disabled', true);
        } else {
            additionalStepBlock.find('.btn-red').attr('disabled', false);
        }
        // onclick
        additionalTC.on('click', function() {
            if (! additionalTC.is(':checked') || ! additionalBehalfOfCompany.is(':checked')) {
                additionalStepBlock.find('.btn-red').attr('disabled', true);
            } else {
                additionalStepBlock.find('.btn-red').attr('disabled', false);
        ...
        // onclick
        additionalBehalfOfCompany.on('click', function() {
            if (! additionalTC.is(':checked') || ! additionalBehalfOfCompany.is(':checked')) {
                additionalStepBlock.find('.btn-red').attr('disabled', true);
            } else {
                additionalStepBlock.find('.btn-red').attr('disabled', false);
...
function validDirectDebitForm(block, paymentMethods){
    ...
    var depositAgreement = block.find('.depositAgreement').is(':checked');
    var paymentOptionsDecision = block.find('.paymentOptionsDecision').is(':checked');
    var paymentOptionsTC = block.find('.paymentOptionsTC').is(':checked');
    if (methods.indexOf(true) != -1 && depositAgreement && paymentOptionsDecision && paymentOptionsTC) {
...
function setEventsMustRemoveBlock() { // it will be removed in future
    let content = $j('.step-content');
    let paymentOptionsBlock = content.find('.payment-options');
    let depositAgreement = paymentOptionsBlock.find('.form-check-input.depositAgreement');
    let behalfOfCompany = paymentOptionsBlock.find('.form-check-input.paymentOptionsDecision');
    let termsAndConditions = paymentOptionsBlock.find('.form-check-input.paymentOptionsTC');

    // onload
    if (! depositAgreement.is(':checked') || ! behalfOfCompany.is(':checked') || ! termsAndConditions.is(':checked')) {
        paymentOptionsBlock.find('.btn-red').attr('disabled', true);
    } else {
        paymentOptionsBlock.find('.btn-red').attr('disabled', false);
    }
    // onclick
    depositAgreement.on('click', function() {
        if (! depositAgreement.is(':checked') || ! behalfOfCompany.is(':checked') || ! termsAndConditions.is(':checked')) {
            paymentOptionsBlock.find('.btn-red').attr('disabled', true);
        } else {
            paymentOptionsBlock.find('.btn-red').attr('disabled', false);
    ...
    // onclick
    behalfOfCompany.on('click', function() {
        if (! depositAgreement.is(':checked') || ! behalfOfCompany.is(':checked') || ! termsAndConditions.is(':checked')) {
            paymentOptionsBlock.find('.btn-red').attr('disabled', true);
        } else {
            paymentOptionsBlock.find('.btn-red').attr('disabled', false);
...
function setupChatInitialization(urlChat, deploymentID, organizationID, buttonID, contactID, opportunityID, accountID, chatFirstMessage, activeStep, activeSubStep, mode){
    ...
    liveagent.addCustomDetail("Chat country", getCountryParameter(), false).saveToTranscript('Country__c');
...
function getCountryParameter(){
    var urlParams = window.location.search.toUpperCase();
    ...
    for (var i = 0; i < params.length; i++) {
        ...
        if (key == 'COUNTRY'){
           var countryName;
           if(value == 'CHDE') countryName = 'Switzerland-DE'
           else if (value == 'CHFR') countryName = 'Switzerland-FR'
           return countryName;
        }
    }
    return '';
...
function turnOffBackButtonClick() {
    allowBackButtonClick = false;
}

function turnOnBackButtonClick() {
    allowBackButtonClick = true;
}

function initPopstateListner() {
    history.pushState(null, null, location.search);
    window.addEventListener('popstate', function(event) {
        if (allowBackButtonClick) {
            var backButtons = document.getElementsByClassName("back-button-cls")
