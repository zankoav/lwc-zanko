...
var regs = {
    ...
    ibanFormat_DE: /^[Dd]{1}[Ee]{1}[0-9]{2}\s?[0-9]{4}\s?[0-9]{4}\s?[0-9]{4}\s?[0-9]{4}\s?[0-9]{2}?$/,
    vatIdFormat_PL: /^([Pp]{1}[Ll]{1}[0-9]{10})+$/,
...
var displayVoucherLightbox;

var dataSource = 'eurolocator.fleetcor.pl';
var favStationInfo = '';
var errors = {
...
$j(document).ready(function () {
    ...
    displayVoucherLightbox = true;

    /* INIT TOP HEADING VIEWPORT */
    ...
    phoneInput.inputmask({mask:"+4\\8999999999", "placeholder": "" }); //using jqueryInputMask.resource
    ...
    if (taxIdInput.val() != '' && taxIdInput.val() != undefined) {
        isInputValueValid(taxIdInput, new RegExp(regs.numbers));
    }
    ...
    setEvents_documents();
    setEvents_dateOfBirth();


    /* STEP PAYMENT OPTIONS */

    // TEMP COMMENTED OUT. (Return back with the return of Ingenico)
    // var paymentOptionsBlock = content.find('.payment-options');
    // paymentOptionsBlock.find('.btn-red').attr('disabled', true);

    // var paymentMethods = paymentOptionsBlock.find('.category-link');
    // validPaymentOptionsForm(paymentOptionsBlock, paymentMethods);

    // setEvents_paymentOptions();
...
function validPaymentOptionsForm(block, paymentMethods){
    var methods = [];
...
function createSlider(destination, dispOutput, idInputHidden, startVal, minVal, maxVal){
    $j('#' + destination).slider({
        ...
        stop: function(event, ui) {
            ...
            analytics_onClickSlideSendUI('kreditlimit_' + creditLimitOutput, 'Tankkarte_Form_PaymentMethod');
            recalculateDeposit();
            colorSliderActiveLine();
        }
    });
...
function validateDocumentsStepData(moveToNextStep) {
    var content = $j('.step-content');
    var documentsStepBlock = content.find('.docoments-agreement');
    var allAgreements = documentsStepBlock.find('input[type="checkbox"].form-check-input');
    ...
    if(confirmedAgreements.indexOf(false) != -1) {
        documentsStepBlock.find('.btn-red.continue').attr('disabled', true);
    }
    allAgreements.click(function() {
        ...
        if(confirmedAgreements.indexOf(false) == -1){
            documentsStepBlock.find('.btn-red.continue').attr('disabled', false);
        } else {
            documentsStepBlock.find('.btn-red.continue').attr('disabled', true);
        }
    });
    var isNextStepDisabled = documentsStepBlock.find('.btn-red.continue').is('[disabled=disabled]');
    if (!isNextStepDisabled && moveToNextStep) {
        saveDocumentsStepData();
    }
}
...
function printPaymentInformation(){
    ...
    params += ', status=no';
    params += ', toolbar=no';
    var paymentInfoPrint = window.open('', 'new div', params);
    paymentInfoPrint.document.write('<html><head><title>Dane dotyczące przelewu bankowego</title>');
    /*optional stylesheet*/ //for example: paymentInfoPrint.document.write('<link rel="stylesheet" href="pay-info.css" type="text/css" />');
    paymentInfoPrint.document.write('</head><body>');
    var printContents = document.getElementById('paymentInformation').innerHTML;
    paymentInfoPrint.document.write(printContents);
    paymentInfoPrint.document.write('</body></html>');
    paymentInfoPrint.print();
    paymentInfoPrint.close();
    window.opener.location.href = none;
}
...
function setEvents_contactDetails() {
    ...
    // set ivent onload
    if (! phoneContactAgreement.is(':checked') || ! infoByEmailAgreement.is(':checked')){
        contactDetailsStepBlock.find('.btn-red').attr('disabled', true);
    }
    ...
    // If Agreement checkbox seleted and all fields are valid => enable Continue button
...
function setEvents_additional() {
    var content = $j('.step-content');
    var additionalStepBlock = content.find('.additional-page');
    var vatRecoveryAgreement = additionalStepBlock.find('.form-check-input.vatRecovery');
    var vatTermsAndConditionsAgreement = additionalStepBlock.find('.form-check-input.vatRecoveryTC');
    if((vatRecoveryAgreement.is(':checked') && ! vatTermsAndConditionsAgreement.is(':checked')) ||
        (! vatRecoveryAgreement.is(':checked') && vatTermsAndConditionsAgreement.is(':checked'))) {
...
function setEvents_vatNumber() {
    ...
    vatNumberInput.on('input', function(e){
        isInputValueValid(vatNumberInput, new RegExp(regs.vatIdFormat_PL));
        isVatValid = isInputValueValid(vatNumberInput, new RegExp(regs.vatIdFormat_PL));
        if (isVatValid) {
...
function setEvents_taxNumber() {
    ...
    taxNumberInput.on('input', function(e){
        ...
        isTaxValid = validateTaxId(taxNumberInput, new RegExp(regs.numbers));
        if (isTaxValid) {
...
function setEvents_documents() {
    validateDocumentsStepData(false);
}


function setEvents_dateOfBirth() {
    var content = $j('.step-content');
    var inputDateOfBirthBlock = content.find('.enter-date-of-birth');
    inputDateOfBirthBlock.find('.btn-red').attr('disabled', true);
    var isDateOfBirth = false;

    var dateOfBirthInput = inputDateOfBirthBlock.find('.date-of-birth').find('input');
    if (dateOfBirthInput.val() != undefined && dateOfBirthInput.val() != '') {
        isDateOfBirth = validateDateOfBirth(dateOfBirthInput, new RegExp(regs.dateOfBirth));
        if (isDateOfBirth) {
            inputDateOfBirthBlock.find('.btn-red').attr('disabled', false);
        } else {
            inputDateOfBirthBlock.find('.btn-red').attr('disabled', true);
        }
    }
...
function validateDateOfBirth(element, regex) {
    var isValid = false;
    var elementValue = element.val();
    if((elementValue.trim() != '') && regex.test(element.val())){
...
function setEvents_paymentOptions(){
    var content = $j('.step-content');
    var paymentOptionsBlock = content.find('.payment-options');
