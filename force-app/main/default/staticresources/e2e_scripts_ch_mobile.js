...
var regs = {
    ...
    lettersAndNumbers: /^[0-9a-zA-ZГЂ-Еѕ\x7f-\xff\s'&.,-]+$/,
...
function setEvents_contactDetails() {
    ...
    $j(contactDetailsStepBlock).find('input:text').each(function(){
        ...
        if ( $j(this).attr('id').indexOf('phone') >= 0) {
            $j(this).inputmask({mask:"+4\\1999999999999", "placeholder": "" });
    ...
    // If Agreement checkbox seleted and all fields are valid РІР‚вЂњ enable Continue button
...
function setEvents_companyDetails() {
    ...
    var creditScoring_requiredInputsCompanyAndCity = creditScoringStepBlock.find('.form-control.required.companyAndCity');
    ...
    $j(creditScoringStepBlock).find('input:text').each(function(){
        ...
        if ( $j(this).val() != '' && $j(this).val() != undefined ) {
            ...
            if ( $j(this).attr('id').indexOf('city') >= 0) {
                isInputValueValid($j(this), new RegExp(regs.letters));
            } else
    ...
    // If no errors in Company and City and display Search companies button
    checkRequiredFieldsAndToggleButton(creditScoring_requiredInputsCompanyAndCity, creditScoringStepBlock.find('.btn-gray.search'));
    ...
    creditScoring_requiredInputsCompanyAndCity.each(function(){
        if ( $j(this).attr('id').indexOf('company') >= 0) {
            $j(this).on('input', function (e){
                isInputValueValid($j(this), new RegExp(regs.lettersAndNumbers));
                checkInputNotEmpty($j(this));
                ...
                if ( isRadioButtonSelected(cfCompanies) ) {
                    checkRequiredFieldsAndToggleButton(creditScoring_requiredInputsCompanyAndCity, creditScoringStepBlock.find('.btn-red.continue'));
                }
            });
        } else
        if ( $j(this).attr('id').indexOf('city') >= 0) {
    ...
    creditScoring_requiredInputTax.on('input', function(e){
        if ($j(this).val().length > 12) {
            $j(this).val($j(this).val().substr(0, 12));
        }
...
function setEvents_cardsOrder(){
    ...
    if (cardTechParamsItems.find('.card-owner').find('input').val() != '' &&
        ...
        if (selectedCardOwnerType != undefined ) {
            if (selectedCardOwnerType.toLowerCase() === 'car-number') {
                validateCarNumber(cardTechParamsItems.find('.card-owner').find('input'), new RegExp(regs.carNumberFormat_DE));
            }
    ...
    cardTechParamsItems.find('.card-owner').find('input').on('input', function() {
        ...
        if (selectedCardOwnerType != undefined ) {
            checkInputNotEmpty($j(this));
            if (selectedCardOwnerType.toLowerCase() === 'car-number') {
                validateCarNumber($j(this), new RegExp(regs.carNumberFormat_DE));
            }
            if (selectedCardOwnerType.toLowerCase() === 'driver-name') {
    ...
    // Add validation on "input" event. VAT Number should in valid format.
    // Required
    // Get Cards Order Block
    vatNumberInput.on('input', function(e){
        validateVatNumber(vatNumberInput, new RegExp(regs.vatIdFormat));
        checkInputNotEmpty(vatNumberInput);
        checkRequiredFieldsAndToggleButton(vatNumberInput, vatNumberBlock.find('.continue-fromvat'));
    });
    // <<< END ELEMENTS EVENTS <<<
...
function setEvents_specialOffers(){
    ...
    window.addEventListener('message', function(event) {
        if (~event.origin.indexOf(dataSource)>=0) {
...
function setEvents_directDebit(){
    ...
    paymentDirectDebit_requiredInputs.each(function(){
        ...
        if ( $j(this).val() != '' && $j(this).val() != undefined ) {
            ...
            if ( $j(this).attr('id').indexOf('IBAN') >= 0) {
                isInputValueValid($j(this), new RegExp(regs.ibanFormat));
            } else
    ...
    if ( ! confirmedAgreements.indexOf(false) >=0 && acceptSepaButton == 'true' ) {
...
function setEvents_bankTransferNoDeposit(){
    ...
    if (confirmedAgreements.indexOf(false)>=0 ) {
...
function setEventsMustRemoveBlock() { // it will be removed in future
    let content = $j('.step-content');
    let paymentOptionsBlock = content.find('.payment-options-block');
...
function validateCompanyDetailsForSearch(){
    ...
    var cityInput = currentStepBlock.find('.city-id').find('input');
    ...
    if (cityInput.val() != '' && cityInput.val() != undefined) {
        isInputValueValid(cityInput, new RegExp(regs.letters));
    }
    isValid = validatedFields.indexOf(false)>=0 ? false : true;
...
function validateCreditLimit(){
    ...
    var minCreditValue;
    var maxCreditValue;
    if (moveToNextStep.html() != undefined) {
        limit = (moveToNextStep.html() === '' ? 0 : parseInt(moveToNextStep.html()));
        minCreditValue = parseInt($j('.slider-label-first').html());
        maxCreditValue = parseInt($j('.slider-label-last').html());
    } else
    if (currentStepBlock.find($j('.limit-value')).html() != undefined) {
        limit = parseInt(currentStepBlock.find($j('.limit-value')).html());
        minCreditValue = limit;
        maxCreditValue = limit;
    }
    ...
    if (deposit != undefined && deposit != '') {
        analytics_PaymentMethodOnSelect($j('[id$=payMethod-dflt]').val(), '1');
    } else {
        analytics_PaymentMethodOnSelect($j('[id$=payMethod-dflt]').val(), '0');
    }
...
function validateEmbossingParams() {
    ...
    if(cardOwnerInput.val() != '' && cardOwnerInput.val() != undefined){
        if(!(selectedCardOwnerType == undefined)){
            if(selectedCardOwnerType.toLowerCase() === 'car-number'){
                isValid = validateCarNumber(cardOwnerInput, new RegExp(regs.carNumberFormat_DE));
            }
            if(selectedCardOwnerType.toLowerCase() === 'driver-name'){
