...
var regs = {
    ...
    email: /^\w+([.-]\w+)*@\w+([.-]\w+)*(\.\w{2,4})+$/,
    carNumberFormat_FR: /^([0-9a-zA-Z-.]{1,9})+$/,
    swiftFormat: /^[A-Za-z]{4}[A-Za-z]{2}[A-Za-z0-9]{2}([A-Za-z0-9]{3})?$/,
    ibanFormat_FR: /^[Ff]{1}[Rr]{1}[0-9]{25}?$/,
    vatIdFormat_FR: /^([Ff]{1}[Rr]{1}[0-9A-Z]{2}[0-9]{9})+$/
};
...
var dataSource = 'eurolocator.fleetcor.fr';
var favStationInfo = '';
var errors = {
...
$j(document).ready(function () {
    ...
    phoneInput.inputmask({mask:"+3\\39999999999", "placeholder": "" }); //using jqueryInputMask.resource
        ...
        if(cardOwnerInput.val() != '' && cardOwnerInput.val() != undefined){
            if(!(selectedCardOwnerType == undefined)){
                if(selectedCardOwnerType.toLowerCase() === 'car-number'){
                    validateCarNumber(cardOwnerInput, new RegExp(regs.carNumberFormat_FR));
                }
                if(selectedCardOwnerType.toLowerCase() === 'driver-name'){
    ...
    if (vatIdInput.val() != '' && vatIdInput.val() != undefined) {
        validateVatId(vatIdInput, content.find('.enter-vat-id').find('[id$="taxIdHidden"]').val());
    }
    ...
    if(ibanInput.val() != '' && ibanInput.val() != undefined) {
        isInputValueValid(ibanInput, new RegExp(regs.ibanFormat_FR));
    }
    ...
    if(cardOwnerInput.val() != '' && cardOwnerInput.val() != undefined){
        if(!(selectedCardOwnerType == undefined)){
            if(cardOwnerInput.val() != ''){
                if(selectedCardOwnerType.toLowerCase() === 'car-number'){
                    *//*isInputValueValid(cardOwnerInput, new RegExp(regs.carNumberFormat_FR));*//*
                    validateCarNumber(cardOwnerInput, new RegExp(regs.carNumberFormat_FR));
                }
                if(selectedCardOwnerType.toLowerCase() === 'driver-name'){
...
function checkCardTechParams() {
    ...
    if(cardOwnerInput.val() != '' && cardOwnerInput.val() != undefined){
        if(!(selectedCardOwnerType == undefined)){
            if(selectedCardOwnerType.toLowerCase() === 'car-number'){
                isCardOwnerValid = validateCarNumber(cardOwnerInput, new RegExp(regs.carNumberFormat_FR));
            }
            if(selectedCardOwnerType.toLowerCase() === 'driver-name'){
...
function validateCardsTechParams(index) {
    ...
    if(cardOwnerInput.val() != undefined){
        if(selectedCardOwnerType != undefined){
            if(selectedCardOwnerType.toLowerCase() === 'car-number'){
                validatedFields.push(validateCarNumber(cardOwnerInput, new RegExp(regs.carNumberFormat_FR)));
            }
            if(selectedCardOwnerType.toLowerCase() === 'driver-name'){
...
function validDirectDebitForm(block, paymentMethods){
    ...
    //if(methods.indexOf(true) != -1 && isAgreementChecked){ //  <!-- TEMP BLOCK (MUST BE ENABLED)  -->
...
function printDocument(frameId) {
    var contentFullSize = $j('#' + frameId).contents().find('.mainform').find('.mCSB_container').height();
    var originalScrollPosition = $j('#' + frameId).contents().find('.mainform').find('.mCSB_container').css('top');
    var originalContentSize = $j('#' + frameId).contents().find('.mainform').css('height');
    // set full iframe size and set scroll to top
    $j('#' + frameId).contents().find('.mainform').attr('style','height: ' + contentFullSize);
    $j('#' + frameId).contents().find('.mainform').find('.mCustomScrollBox').attr('style','max-height: ' + contentFullSize);
    ...
    // back to original iframe size and original scroll possition
    $j('#' + frameId).contents().find('.mainform').attr('style','height: ' + originalContentSize);
    $j('#' + frameId).contents().find('.mainform').find('.mCustomScrollBox').attr('style','max-height: ' + originalContentSize);
    $j('#' + frameId).contents().find('.mCSB_container').attr('style', 'position: relative; top: '+ originalScrollPosition +'; left: 0px;');
}
...
function printPaymentInformation(){
    ...
    paymentInfoPrint.document.write('<html><head><title>Coordonnées de virement bancaire</title>');
...
function setEvents_vatNumber() {
    ...
    vatNumberInput.on('input', function(e){
        isInputValueValid(vatNumberInput, new RegExp(regs.vatIdFormat_FR));
        isVatValid = isInputValueValid(vatNumberInput, new RegExp(regs.vatIdFormat_FR));
        if (isVatValid) {
...
function validateVatId(element, taxId) {
    var validVatId;
    if (taxId.length == 9) {
        validVatId = 'FR' + taxId;
        var validationKey = (12 + 3 * (taxId % 97)) % 97;
        validVatId = 'FR' + validationKey + taxId;
    } else {
        validVatId = taxId;
    }

    if (element.val().trim() == validVatId) {
        element.closest('.form-group input').removeClass('error');
        element.closest('.form-group input').addClass('success');
        element.closest('.form-group').find('.invalid-feedback').hide();
        element.closest('.form-group').find('.glyphicon').show();
