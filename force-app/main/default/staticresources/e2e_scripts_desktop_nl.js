...
var dataSource = 'eurolocator.fleetcor.nl';
var favStationInfo = '';
var errors = {
...
$j(document).ready(function () {
    ...
    phoneInput.inputmask({mask:"+3\\1999999999999", "placeholder": "" }); //using jqueryInputMask.resource
        ...
        if(cardOwnerInput.val() != '' && cardOwnerInput.val() != undefined){
            if(!(selectedCardOwnerType == undefined)){
                if(selectedCardOwnerType.toLowerCase() === 'car-number'){
                    validateCarNumber(cardOwnerInput, new RegExp(regs.carNumberFormat_NL));
                }
                if(selectedCardOwnerType.toLowerCase() === 'driver-name'){
    ...
    if (vatIdInput.val() != '' && vatIdInput.val() != undefined) {
        isInputValueValid(vatIdInput, new RegExp(regs.vatIdFormat_NL));
    }
    ...
    if(ibanInput.val() != '' && ibanInput.val() != undefined) {
        isInputValueValid(ibanInput, new RegExp(regs.ibanFormat_NL));
    }
    ...
    if(cardOwnerInput.val() != '' && cardOwnerInput.val() != undefined){
        if(!(selectedCardOwnerType == undefined)){
            if(cardOwnerInput.val() != ''){
                if(selectedCardOwnerType.toLowerCase() === 'car-number'){
                    *//*isInputValueValid(cardOwnerInput, new RegExp(regs.carNumberFormat_NL));*//*
                    validateCarNumber(cardOwnerInput, new RegExp(regs.carNumberFormat_NL));
                }
                if(selectedCardOwnerType.toLowerCase() === 'driver-name'){
...
function checkCardTechParams() {
    ...
    if(cardOwnerInput.val() != '' && cardOwnerInput.val() != undefined){
        if(!(selectedCardOwnerType == undefined)){
            if(selectedCardOwnerType.toLowerCase() === 'car-number'){
                isCardOwnerValid = validateCarNumber(cardOwnerInput, new RegExp(regs.carNumberFormat_NL));
            }
            if(selectedCardOwnerType.toLowerCase() === 'driver-name'){
...
function validateCardsTechParams(index) {
    ...
    if(cardOwnerInput.val() != undefined){
        if(selectedCardOwnerType != undefined){
            if(selectedCardOwnerType.toLowerCase() === 'car-number'){
                validatedFields.push(validateCarNumber(cardOwnerInput, new RegExp(regs.carNumberFormat_NL)));
            }
            if(selectedCardOwnerType.toLowerCase() === 'driver-name'){
...
function validateCarNumber(element, regex) {
    ...
    var elementValue = element.val();
    var lastChar = elementValue.charAt(numLength - 1);
...
function printPaymentInformation(){
    ...
    paymentInfoPrint.document.write('<html><head><title>Informatie voor de overschrijving</title>');
...
function setEvents_vatNumber() {
    ...
    vatNumberInput.on('input', function(e){
        isInputValueValid(vatNumberInput, new RegExp(regs.vatIdFormat_NL));
        isVatValid = isInputValueValid(vatNumberInput, new RegExp(regs.vatIdFormat_NL));
        if (isVatValid) {
...
function setEvents_directDebit(){
    ...
    if (ibanInput.val() != '' && ibanInput.val() != undefined) {
        isInputValueValid(ibanInput, new RegExp(regs.ibanFormat_BENL));
    }
...
function setupOfflineChat(){
    ...
    chatOfflineOLDText = '';
    chatOfflineUpdateTimeout = setTimeout(updateDatabaseOfflineChat, 10000);
}
