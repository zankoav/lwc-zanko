...
var regs = {
    ...
    // email: /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/,
    email: /^\w+([.-]\w+)*@\w+([.-]\w+)*(\.\w{2,4})+$/,
    carNumberFormat_AT: /^[A-Za-z]{1,3}\-?\s?[A-Za-z]{0,2}[0-9]{1,4}[H]{0,1}/,
    swiftFormat: /^[A-Za-z]{4}[A-Za-z]{2}[A-Za-z0-9]{2}([A-Za-z0-9]{3})?$/,
    ibanFormat_AT: /^[Aa]{1}[Tt]{1}[0-9]{2}\s?[0-9]{4}\s?[0-9]{4}\s?[0-9]{4}\s?[0-9]{4}?$/,
    vatIdFormat_AT: /^([Aa]{1}[Tt]{1}[Uu]{1}[0-9]{8})+$/,
...
var dataSource = 'eurolocator.fleetcor.at';
var favStationInfo = '';
var errors = {
...
$j(document).ready(function () {
    ...
    phoneInput.inputmask({mask:"+4\\399999999999999", "placeholder": "" }); //using jqueryInputMask.resource
    ...
    taxIdInput.inputmask({mask:"99-9999999", "placeholder": "" });
        ...
        if(cardOwnerInput.val() != '' && cardOwnerInput.val() != undefined){
            if(!(selectedCardOwnerType == undefined)){
                if(selectedCardOwnerType.toLowerCase() === 'car-number'){
                    validateCarNumber(cardOwnerInput, new RegExp(regs.carNumberFormat_AT));
                }
                if(selectedCardOwnerType.toLowerCase() === 'driver-name'){
    ...
    if (vatIdInput.val() != '' && vatIdInput.val() != undefined) {
        isInputValueValid(vatIdInput, new RegExp(regs.vatIdFormat_AT));
    }
    ...
    if(ibanInput.val() != '' && ibanInput.val() != undefined) {
        isInputValueValid(ibanInput, new RegExp(regs.ibanFormat_AT));
    }
    ...
    if(cardOwnerInput.val() != '' && cardOwnerInput.val() != undefined){
        if(!(selectedCardOwnerType == undefined)){
            if(cardOwnerInput.val() != ''){
                if(selectedCardOwnerType.toLowerCase() === 'car-number'){
                    *//*isInputValueValid(cardOwnerInput, new RegExp(regs.carNumberFormat_AT));*//*
                    validateCarNumber(cardOwnerInput, new RegExp(regs.carNumberFormat_AT));
                }
                if(selectedCardOwnerType.toLowerCase() === 'driver-name'){
...
function checkCardTechParams() {
    ...
    if(cardOwnerInput.val() != '' && cardOwnerInput.val() != undefined){
        if(!(selectedCardOwnerType == undefined)){
            if(selectedCardOwnerType.toLowerCase() === 'car-number'){
                isCardOwnerValid = validateCarNumber(cardOwnerInput, new RegExp(regs.carNumberFormat_AT));
            }
            if(selectedCardOwnerType.toLowerCase() === 'driver-name'){
...
function validateCardsTechParams(index) {
    ...
    if(cardOwnerInput.val() != undefined){
        if(selectedCardOwnerType != undefined){
            if(selectedCardOwnerType.toLowerCase() === 'car-number'){
                validatedFields.push(validateCarNumber(cardOwnerInput, new RegExp(regs.carNumberFormat_AT)));
            }
            if(selectedCardOwnerType.toLowerCase() === 'driver-name'){
...
function setEvents_vatNumber() {
    ...
    vatNumberInput.on('input', function(e){
        isInputValueValid(vatNumberInput, new RegExp(regs.vatIdFormat_AT));
        isVatValid = isInputValueValid(vatNumberInput, new RegExp(regs.vatIdFormat_AT));
        if (isVatValid) {
...
function setEvents_taxNumber() {
    ...
    taxNumberInput.on('input', function(e){
        validateTaxId(taxNumberInput, new RegExp(regs.taxId));
...
function setupChatInitialization(urlChat, deploymentID, organizationID, buttonID, contactID, opportunityID, accountID, chatFirstMessage, activeStep, activeSubStep, mode){
    ...
    liveagent.addCustomDetail("Chat country", 'Germany', false).saveToTranscript('Country__c');
