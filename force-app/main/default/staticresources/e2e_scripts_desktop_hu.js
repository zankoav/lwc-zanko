...
var regs = {
    ...
    vatIdFormat_HU: /^([0-9]{8}[-]{1}[1-5]{1}[-]{1}[0-9]{2})+$/,
...
$j(document).ready(function () {
    ...
    phoneInput.inputmask({mask:"+3\\6999999999", "placeholder": "" }); //using jqueryInputMask.resource
    ...
    var inputVatIdBlock = content.find('.tax-id');
    inputVatIdBlock.find('.btn-red').attr('disabled', true);
    var vatIdInput = creditScoringStepBlock.find('.tax-id-field');
    // vatIdInput.inputmask({mask:"99999999-9-99", "placeholder": "________-_-__" });
    $j('#search-companies-block').focus();
    if(vatIdInput.val() != '' && vatIdInput.val() != undefined){
        validateVatId(vatIdInput, new RegExp(regs.vatIdFormat_HU));
    }
...
function validateVatId(element, regex) {
    var isValid = false;
    var vatIdSize = element.val().length;
    var elementValue = element.val();
    if((elementValue.trim() != '') && regex.test(elementValue)){
        if(vatIdSize == 13){
            element.closest('.form-group input').removeClass('error');
...
function validateCreditLimitValue(inputElement) {
    ...
    if ($j('.slider-label-first').html() != undefined && $j('.slider-label-last').html() != undefined) {
        var minCreditValue = parseInt(($j('.slider-label-first').html()).replace(/\s/g,''));
        var maxCreditValue = parseInt(($j('.slider-label-last').html()).replace(/\s/g,''));
...
function printPaymentInformation(){
    ...
    paymentInfoPrint.document.write('<html><head><title>Adatok a banki átutaláshoz</title>');
...
function setEvents_companyDetails() {
    ...
    var inputVatIdBlock = creditScoringStepBlock.find('.tax-id');
    var vatIdInput = creditScoringStepBlock.find('.tax-id-field');
    $j('#search-companies-block').focus();
    if(vatIdInput.val() != '' && vatIdInput.val() != undefined){
        validateVatId(vatIdInput, new RegExp(regs.vatIdFormat_HU));
    }
    vatIdInput.on('input', function(e){
        validateVatId(vatIdInput, new RegExp(regs.vatIdFormat_HU));
        isVatValid = validateVatId(vatIdInput, new RegExp(regs.vatIdFormat_HU));
        if (isVatValid) {
...
function increaseCreditLimit(maxVal) {
    var currentLimit = parseInt($j('[id$=idInputHidden]').val());
    if (!isNaN(currentLimit)) {
        setNewCreditLimit(currentLimit + 10000);
    }
}

function decreaseCreditLimit(minVal, maxVal) {
    var currentLimit = parseInt($j('[id$=idInputHidden]').val());
    if (!isNaN(currentLimit)) {
        setNewCreditLimit(currentLimit - 10000);
    }
...
function setValidCurrencyFormat() {
    $j('.currencyFormat').each(function(index, element){
