...
function setEvents_contactDetails() {
    ...
    $j(contactDetailsStepBlock).find('input:text').each(function(){
        ...
        if ( $j(this).attr('id').indexOf('phone') >= 0) {
            // $j(this).inputmask({mask:"+4\\8999999999999", "placeholder": "" });
    ...
    $j('input.input').bind('keydown', function(event) {
        if (event.which === 13) {
            event.stopPropagation();
            event.preventDefault();
            $j(':input.input:eq(' + ($j(':input.input').index(this) + 1) +')').focus();
        }
    });    
...
function setEvents_companyDetails() {
    ...
    if ( $j(creditScoringStepBlock) != undefined && $j(creditScoringStepBlock).attr('id') != undefined && $j(creditScoringStepBlock).attr('id').indexOf('pump') >= 0) {
        creditScoringStepBlock.find('.btn-red.search').attr('disabled', true);
    } else 
    if ($j(creditScoringStepBlock) != undefined && $j(creditScoringStepBlock).attr('id') != undefined) {
        creditScoringStepBlock.find('.btn-red.search').attr('disabled', false);
    }
    ...
    creditScoring_requiredInputTax.on('input', function(e){
        if ($j(this).val().length > 10) {
            $j(this).val($j(this).val().substr(0, 10));
        }
        var isTaxValid = validateTaxId($j(this), new RegExp(regs.taxid));
        if (isTaxValid) {
            creditScoringStepBlock.find('.btn-red.search').attr('disabled', false);
        } else {
            creditScoringStepBlock.find('.btn-red.search').attr('disabled', true);
...
function setEvents_cardsOrder(){
    ...
    if (cardTechParamsItems.find('.card-owner').find('input').val() != '' &&
        ...
        if (selectedCardOwnerType != undefined ) {
            if (selectedCardOwnerType.toLowerCase() === 'car-number') {
                validateCarNumber(cardTechParamsItems.find('.card-owner').find('input'), new RegExp(regs.carNumberFormat_NL));
            }
    ...
    cardTechParamsItems.find('.card-owner').find('input').on('input', function() {
        ...
        if (selectedCardOwnerType != undefined ) {
            checkInputNotEmpty($j(this));
            if (selectedCardOwnerType.toLowerCase() === 'car-number') {
                validateCarNumber($j(this), new RegExp(regs.carNumberFormat_NL));
            }
            if (selectedCardOwnerType.toLowerCase() === 'driver-name') {
    ...
    // Collapse block event for card configuration fees and costs details
    $j('.collapse-block .link').on('click', function(){
        var detailsBlock = $j(this).parent().find('#feesAndCostsTop');
        var collapseBlock = $j(this).find('.fees-and-cost-expander');
        if(detailsBlock.is(':hidden')){
            collapseBlock.removeClass('block-expand').addClass('block-collapse');
        } else {
            collapseBlock.removeClass('block-collapse').addClass('block-expand');
    ...
    paymentDirectDebit_requiredInputs.each(function(){
        ...
        if ( $j(this).val() != '' && $j(this).val() != undefined ) {
            ...
            if ( $j(this).attr('id').indexOf('IBAN') >= 0) {
                isInputValueValid($j(this), new RegExp(regs.ibanFormat_NL));
            } else
...
function setEvents_paymentOptions() {
    ...
    // var content = $j('.step-content');
    // var paymentOptionsBlock = content.find('.payment-options-block');
    // var paymentMethods = paymentOptionsBlock.find('.category-link');
    // if (paymentMethods.hasClass('active')) {
    //     paymentOptionsBlock.find('.btn-red.continue').attr('disabled', false);
    // } else {
    //     paymentOptionsBlock.find('.btn-red.continue').attr('disabled', true);
    // }
...
function validateCreditLimit(moveToNextStep, companyForm) {
    ...
    if (deposit != undefined && deposit != '') {
        analytics_PaymentMethodOnSelect($j('[id$=payMethod-dflt]').val(), '1', companyForm);
    } else {
        analytics_PaymentMethodOnSelect($j('[id$=payMethod-dflt]').val(), '0', companyForm);
    }
...
function validateEmbossingParams() {
    ...
    if(cardOwnerInput.val() != '' && cardOwnerInput.val() != undefined){
        if(!(selectedCardOwnerType == undefined)){
            if(selectedCardOwnerType.toLowerCase() === 'car-number'){
                isValid = validateCarNumber(cardOwnerInput, new RegExp(regs.carNumberFormat_NL));
            }
            if(selectedCardOwnerType.toLowerCase() === 'driver-name'){
...
function validDirectDebitForm(block, paymentMethods){
    ...
    if (methods.indexOf(true)>=0 && isAgreementChecked){
...
function validateDocumentsStepData(moveToNextStep) {
    var documentsStepBlock = $j('.step-content').find('.documents-agreement');
    var decisionCheckbox = documentsStepBlock.find('.form-check-input.decision');
    var termsAndConditionsCheckbox = documentsStepBlock.find('.form-check-input.termsAndConditions');

    if ( ! decisionCheckbox.is(':checked') && ! termsAndConditionsCheckbox.is(':checked')) {
        documentsStepBlock.find('.btn-red').attr('disabled', true);
    }

    decisionCheckbox.on('click', function() {
        if (decisionCheckbox.is(':checked') && termsAndConditionsCheckbox.is(':checked')) {
            documentsStepBlock.find('.btn-red').attr('disabled', false);
        } else {
            documentsStepBlock.find('.btn-red').attr('disabled', true);
        }
    });
    ...
    var isNextStepDisabled = documentsStepBlock.find('.btn-red.sign').is('[disabled=disabled]');
