...
function setEvents_contactDetails() {
    ...
    $j(contactDetailsStepBlock).find('input:text').each(function(){
        ...
        if ( $j(this).attr('id').indexOf('phone') >= 0) {
            // $j(this).inputmask({mask:"+3\\1999999999999", "placeholder": "" });
        ...
        if ( $j(this).attr('id').indexOf('tax') >= 0) {
            $j(this).on('input', function (e) {
                ...
                if ( $j(this).val().length != 8 ) {
                    creditScoringStepBlock.find('.btn-red.search').addClass('disabled');
                } else {
                    creditScoringStepBlock.find('.btn-red.search').removeClass('disabled');
                }
            });
        }
    });
