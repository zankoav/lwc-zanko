...
var regs = {
    ...
    //email: /^([\w-\.]+@([\w-]+\.)+[\w-]{2,4})?$/,
    email: /^\w+([.-]\w+)*@\w+([.-]\w+)*(\.\w{2,4})+$/,
    carNumberFormat_BENL: /^([0-9a-zA-Z-.]{1,9})+$/,
    swiftFormat: /^[A-Za-z]{4}[A-Za-z]{2}[A-Za-z0-9]{2}([A-Za-z0-9]{3})?$/,
    ibanFormat_BENL: /^[Bb]{1}[Ee]{1}[0-9]{14}?$/,
    vatIdFormat_BENL: /^[Bb]{1}[Ee]{1}[0-9]{9,10}?$/,
...
$j(document).ready(function () {
    ...
    phoneInput.inputmask({mask:"+3\\29999999999", "placeholder": "" }); //using jqueryInputMask.resource
        ...
        if(cardOwnerInput.val() != '' && cardOwnerInput.val() != undefined){
            if(!(selectedCardOwnerType == undefined)){
                if(selectedCardOwnerType.toLowerCase() === 'car-number'){
                    validateCarNumber(cardOwnerInput, new RegExp(regs.carNumberFormat_BENL));
                }
                if(selectedCardOwnerType.toLowerCase() === 'driver-name'){
    ...
    if(cardOwnerInput.val() != '' && cardOwnerInput.val() != undefined){
        if(!(selectedCardOwnerType == undefined)){
            if(cardOwnerInput.val() != ''){
                if(selectedCardOwnerType.toLowerCase() === 'car-number'){
                    *//*isInputValueValid(cardOwnerInput, new RegExp(regs.carNumberFormat_BENL));*//*
                    validateCarNumber(cardOwnerInput, new RegExp(regs.carNumberFormat_BENL));
                }
                if(selectedCardOwnerType.toLowerCase() === 'driver-name'){
...
function checkCardTechParams() {
    ...
    if(cardOwnerInput.val() != '' && cardOwnerInput.val() != undefined){
        if(!(selectedCardOwnerType == undefined)){
            if(selectedCardOwnerType.toLowerCase() === 'car-number'){
                isCardOwnerValid = validateCarNumber(cardOwnerInput, new RegExp(regs.carNumberFormat_BENL));
            }
            if(selectedCardOwnerType.toLowerCase() === 'driver-name'){
...
function validateCardsTechParams(index) {
    ...
    if(cardOwnerInput.val() != undefined){
        if(selectedCardOwnerType != undefined){
            if(selectedCardOwnerType.toLowerCase() === 'car-number'){
                validatedFields.push(validateCarNumber(cardOwnerInput, new RegExp(regs.carNumberFormat_BENL)));
            }
            if(selectedCardOwnerType.toLowerCase() === 'driver-name'){
...
function setEvents_vatNumber() {
    ...
    vatNumberInput.on('input', function(e){
        isInputValueValid(vatNumberInput, new RegExp(regs.vatIdFormat_BENL));
        isVatValid = isInputValueValid(vatNumberInput, new RegExp(regs.vatIdFormat_BENL));
        if (isVatValid) {
