...
analyticsErrorsMap.set('1-Too high limit','Tankkarte_Step_2_Form_Company');
...
analyticsErrorsMap.set('1-Enter Tax ID','Tankkarte_Step_3_Form_FuelLiters');
...
function analytics_anyError(errorSubstep,errorPage,errorText) {
    ...
    'eventAction': errorText, 'eventLabel': analyticsFinalError
    });
}
...
function analytics_onClickOrderButtonSendUI(buttonName, activeSubStep) {
    ...
    } else if (activeSubStep == 'Enter Tax ID' || activeSubStep == 'Set total consumption') {
    ...
    } else if (activeSubStep == 'DirectDebit' || activeSubStep == 'BankTransfer') {
        category = 'UI_09_Tankkarte_Step_9_Payment_Details';
    } else if (activeSubStep == 'DirectDebitDeposit' || activeSubStep == 'BankTransferDeposit') {
